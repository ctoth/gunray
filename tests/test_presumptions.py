...
def test_presumption_holds_when_no_defeating_argument_exists() -> None:
    ...
    model = DefeasibleEvaluator().evaluate(_presumption_only_theory(), Policy.BLOCKING)
...
def test_presumption_challenged_observed_behavior() -> None:
    ...
    """
    model = DefeasibleEvaluator().evaluate(_rebutted_presumption_theory(), Policy.BLOCKING)
